...
pub fn accounts_representatives(raw_json: JsonValue, accounts: &[Account]) -> Result<Vec<Option<Account>>, RpcError> {
...
/// Legacy blocks will return `None`
pub fn blocks_info(raw_json: JsonValue, hashes: &[[u8; 32]]) -> Result<Vec<Option<Block>>, RpcError> {
    let mut blocks = vec!();
    for hash in hashes {
...
pub fn work_generate(raw_json: JsonValue, work_hash: [u8; 32], custom_difficulty: Option<[u8; 8]>) -> Result<[u8; 8], RpcError> {
...
mod tests {
    ...
    #[test]
    fn account_balance() {
        let balance = super::account_balance(
            json!({
                "balance": "10000",
                "pending": "20000",
                "receivable": "30000"
    ...
    fn account_representative() {
        ...
        let representative = super::account_representative(
            vec!(
                Block {
                    block_type: BlockType::Send,
                    account: "nano_1ipx847tk8o46pwxt5qjdbncjqcbwcc1rrmqnkztrfjy5k7z4imsrata9est".try_into().unwrap(),
