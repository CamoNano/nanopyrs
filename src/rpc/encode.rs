...
pub async fn account_balance(account: &Account) -> JsonValue {
...
pub async fn account_history(account: &Account, count: usize, head: Option<[u8; 32]>) -> JsonValue {
    let mut arguments = Map::new();
...
pub async fn accounts_balances(accounts: &[Account]) -> JsonValue {
...
pub async fn accounts_frontiers(accounts: &[Account]) -> JsonValue {
    let accounts: Vec<String> = accounts.iter()
        .map(|account| account.to_string()).collect();
...
pub async fn accounts_receivable(accounts: &[Account], count: usize, threshold: u128) -> JsonValue {
    let accounts: Vec<String> = accounts.iter()
        .map(|account| account.to_string()).collect();
...
pub async fn accounts_representatives(accounts: &[Account]) -> JsonValue {
    let accounts: Vec<String> = accounts.iter()
        .map(|account| account.to_string()).collect();
...
pub async fn block_info(hash: [u8; 32]) -> JsonValue {
    let mut arguments = Map::new();
...
pub async fn blocks_info(hashes: &[[u8; 32]]) -> JsonValue {
...
pub async fn process(block: Block) -> JsonValue {
    let mut arguments = Map::new();
...
pub async fn work_generate(work_hash: [u8; 32], custom_difficulty: Option<[u8; 8]>) -> JsonValue {
    let mut arguments = Map::new();
...
mod tests {
    ...
    fn account_balance() {
        ...
        let json = runtime().block_on(super::account_balance(&account));
        assert!(json == json!({
    ...
    fn account_history() {
        ...
        let json = runtime().block_on(super::account_history(&account, 3, None));
        assert!(json == json!({
        ...
        let json = runtime().block_on(super::account_history(&account, 4, Some([255; 32])));
        assert!(json == json!({
            "action": "account_history",
            "account": "nano_1ipx847tk8o46pwxt5qjdbncjqcbwcc1rrmqnkztrfjy5k7z4imsrata9est",
    ...
    fn accounts_balances() {
        ...
        let json = runtime().block_on(super::accounts_balances(&accounts));
        assert!(json == json!({
    ...
    fn accounts_frontiers() {
        ...
        let json = runtime().block_on(super::accounts_frontiers(&accounts));
        assert!(json == json!({
    ...
    fn accounts_receivable() {
        ...
        let json = runtime().block_on(super::accounts_receivable(&accounts, 9, 1000000000000000000000000));
        assert!(json == json!({
    ...
    fn accounts_representatives() {
        ...
        let json = runtime().block_on(super::accounts_representatives(&accounts));
        assert!(json == json!({
    ...
    fn block_info() {
        ...
        let json = runtime().block_on(super::block_info(hash));
        assert!(json == json!({
            "action": "block_info",
            "json_block": "true",
    ...
    fn blocks_info() {
        ...
        let json = runtime().block_on(super::blocks_info(&hashes));
        assert!(json == json!({
            "action": "blocks_info",
            "json_block": "true",
    ...
    fn process() {
        ...
        let json = runtime().block_on(super::process(block));
        assert!(json == json!({
            "action": "process",
            "json_block": "true",
            "subtype": "send",
    ...
    fn work_generate() {
        ...
        let json = runtime().block_on(super::work_generate(hash, None));
        assert!(json == json!({
        ...
        let hash = hex::decode("718CC2121C3E641059BC1C2CFC45666C99E8AE922F7A807B7D07B62C995D79E2").unwrap().try_into().unwrap();
        let json = runtime().block_on(super::work_generate(hash, Some([255; 8])));
        assert!(json == json!({
            "action": "work_generate",
            "hash": "718CC2121C3E641059BC1C2CFC45666C99E8AE922F7A807B7D07B62C995D79E2",
