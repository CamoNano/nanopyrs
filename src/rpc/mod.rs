...
use internal::{InternalRpc, trim_json};
pub use internal::RpcError;

fn json_keys(value: JsonValue) -> Result<Vec<String>, RpcError> {
...
fn u128_from_json(value: &JsonValue) -> Result<u128, RpcError> {
    Ok(
...
fn bytes_from_json<const T: usize>(value: &JsonValue) -> Result<[u8; T], RpcError> {
    hex::decode(trim_json(value.to_string()))?
        .try_into()
        .or(Err(RpcError::ParseError("failed to parse hex".into())))
}

fn account_from_json(value: &JsonValue) -> Result<Account, RpcError> {
    Ok(Account::try_from(
        &trim_json(value.to_string())
    )?)
}
...
fn block_from_json(block: &JsonValue, block_type: BlockType) -> Result<Block, RpcError> {
    Ok(Block{
        block_type,
...
fn block_from_history_json(block: &JsonValue) -> Result<Block, RpcError> {
...
fn block_from_info_json(block: &JsonValue) -> Result<Block, RpcError> {
...
impl Rpc {
    ...
    pub async fn account_history(&self, account: &Account, count: usize, head: Option<[u8; 32]>) -> Result<Vec<Block>, RpcError> {
        ...
        for block in json_blocks {
            ...
            // must ensure account is correct
    ...
    pub async fn accounts_receivable(&self, accounts: &[Account], count: usize, threshold: u128) -> Result<Vec<Vec<([u8; 32], u128)>>, RpcError> {
        ...
        for account in &accounts {
            ...
            let account_hashes = json_keys(raw_json[&account].clone());
    ...
    pub async fn process(&self, block: Block) -> Result<[u8; 32], RpcError> {
        ...
        json_block.insert("type".into(), "state".into());
        ...
        arguments.insert("block".into(), JsonValue::Object(json_block));
    ...
    pub async fn work_generate(&self, work_hash: [u8; 32]) -> Result<[u8; 8], RpcError> {
        let raw_json = self.internal.work_generate(hex::encode(work_hash), Map::new()).await?;
